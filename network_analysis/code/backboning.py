...
#def target_densities(table, edges)
...
def test_densities(table, start, end, step):
   ...
   for s in steps:
      ...
      try:
         ...
         result.append((s, nodes, (100.0 * nodes) / onodes, edges, (100.0 * edges) / oedges, avgdeg, avgdeg / oavgdeg))
      except ZeroDivisionError:
         break
   return pd.DataFrame(result, columns = ("threshold", "#nodes", "%nodes", "#edges", "%edges", "avgdegree", "%avgdegree"))
...
def main():
   ...
   resultat = test_densities(taula, 3, 3.5, 0.01)
   ...
   #END TARGET: 
   #target treshold 6.67
   final = thresholding(taula, threshold=3.07)
