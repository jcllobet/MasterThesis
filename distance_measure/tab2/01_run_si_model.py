...
def si_step(network, infected, contagion_parameter):
   new_infected = copy.deepcopy(infected)
   for node in network.nodes():
      neighbors = set(network.neighbors(node))
      infected_neighbors = neighbors & set(infected.keys())
      if len(infected_neighbors) > 0 and random.random() < contagion_parameter:
         new_infected[node] = 1
   return new_infected
...
with open("si_table_%s.csv" % network_type, 'w') as f:
